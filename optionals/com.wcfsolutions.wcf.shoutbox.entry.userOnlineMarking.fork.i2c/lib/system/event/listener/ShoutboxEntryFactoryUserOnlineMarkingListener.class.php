		...
		if (MODULE_USERS_ONLINE == 1) {
			if ($eventName == 'shouldInit') {
				...
				foreach ( $eventObj->getEntries () as $entry ) {
					if ($entry->username == WCF::getLanguage ()->get ( 'wcf.shoutbox.bot.name' )) {
						$entry->usernameStyle = WCF::getLanguage ()->get ( 'wcf.shoutbox.bot.style' );
					} elseif ($entry->userID && isset ( $groups [$entry->userOnlineGroupID] ) && $groups [$entry->userOnlineGroupID] ['userOnlineMarking'] != '%s') {
