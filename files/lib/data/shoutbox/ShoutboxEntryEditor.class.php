...
class ShoutboxEntryEditor extends ShoutboxEntry {
	 ...
	 */
	public static function create($userID, $username, $message, $toUserID, $toUserName, $me) {
		$sql = "INSERT INTO	wcf" . WCF_N . "_shoutbox_entry
					(userID, username, time, message,me, toUserID, toUserName, ipAddress)
			VALUES		(" . $userID . ", '" . escapeString ( $username ) . "', " . TIME_NOW . ", '" . escapeString ( $message ) . "', $me ,$toUserID,'" . escapeString ( $toUserName ) . "',  '" . escapeString ( WCF::getSession ()->ipAddress ) . "')";
	...
	public static function unmark($entryIDArray) {
		...
		if ($markedShoutboxEntries !== null) {
			...
			if (count ( $markedShoutboxEntries ))
				WCF::getSession ()->register ( 'markedShoutboxEntries', $markedShoutboxEntries );
			else
