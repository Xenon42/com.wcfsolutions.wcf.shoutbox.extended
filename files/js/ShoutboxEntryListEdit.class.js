 ...
 */
var ShoutboxEntryListEdit = Class
		.create({
			/**
			...
			saveMarkedStatus : function(data) {
				new Ajax.Request('index.php?action=ShoutboxEntryMark&t='
			...
			removeMarked : function() {
				if (confirm(language['wcf.shoutbox.entry.deleteMarked.sure'])) {
					document.location.href = fixURL('index.php?action=ShoutboxEntryDeleteMarked&t='
							+ SECURITY_TOKEN + SID_ARG_2ND);
				}
			},
			 ...
			 */
			unmarkAll : function() {
				new Ajax.Request('index.php?action=ShoutboxEntryUnmarkAll&t='
						+ SECURITY_TOKEN + SID_ARG_2ND, {
					method : 'get'
				});
